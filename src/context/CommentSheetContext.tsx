...
import { CommentItem } from '../components/circle/CommentItem';
...
const CommentSheetContext = createContext<CommentSheetContextValue | undefined>(undefined);
...
export function CommentSheetProvider({ children }: { children: React.ReactNode }) {
  ...
  const onCommentAddedCallback = useRef<((comment: Comment) => void) | undefined>(undefined);
  const sendingRef = useRef(false);

  // Smooth Reanimated keyboard tracking for mobile
  ...
  const openComments = useCallback(
    (
      ...
      onCommentAddedCallback.current = onCommentAdded;
      if (initialComments && initialComments.length > 0) {
        setComments(initialComments);
      } else {
        const matched = mockComments.filter((c) => c.postId === postId);
        setComments(matched);
      }
      setIsOpen(true);
    },
  ...
  const handleSend = () => {
    ...
    if (!trimmed || !activePostId || sendingRef.current) return;
    ...
    onCommentAddedCallback.current?.(newComment);
    setInputText('');
  ...
  return (
    <CommentSheetContext.Provider value={{ openComments, closeComments, isOpen }}>
      {children}

      {/* Top-Level TikTok-Style In-Tree Overlay */}
      {isOpen && (
        <View
          style={[
            styles.overlayRoot,
        ...
        >
          ...
          {/* Reanimated Animated Sheet Container */}
          <Animated.View
            style={[
              styles.sheetBase,
          ...
          >
            ...
            <View style={[styles.header, { borderBottomColor: colors.divider }]}>
              ...
              <Pressable onPress={closeComments} hitSlop={12} style={styles.closeButton}>
